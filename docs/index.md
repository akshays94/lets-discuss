...
```
heroku create letsdiscuss-prod --remote prod && \
    heroku addons:create newrelic:wayne --app letsdiscuss-prod && \
    heroku addons:create heroku-postgresql:hobby-dev --app letsdiscuss-prod && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="letsdiscuss-prod" \
        --app letsdiscuss-prod
```

Creating the qa sever:

```
heroku create `letsdiscuss-qa --remote qa && \
    heroku addons:create newrelic:wayne && \
    heroku addons:create heroku-postgresql:hobby-dev && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="letsdiscuss-qa" \
```
